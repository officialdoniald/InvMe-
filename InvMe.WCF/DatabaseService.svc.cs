...
namespace InvMe.WCF
{
    ...
    {
        public static string ConnectionString { get; } = Properties.Settings.Default.ConnectionString;
        ...
        public string INSERT_EVENTS_SQL { get; } =
            ...
            "([EVENTNAME], [DESCRIPTION], [FROM], [TO], [ONLINE], [TOWN], [PLACE], [MDESCRIPTION], [HOWMANY], [MEETINGCORD], [PLACECORD]) " +
            "VALUES(" +
            "@EVENTNAME,@DESCRIPTION,@FROM,@TO,@ONLINE,@TOWN,@PLACE,@MDESCRIPTION,@HOWMANY,@MEETINGCORD,@PLACECORD);SET @id = SCOPE_IDENTITY();";
        ...
        public List<HashtagsM> GetHashtag()
        {
            List<HashtagsM> hashtags = new List<HashtagsM>();
        ...
        public int InsertEvents(Events events)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(ConnectionString))
                {
                    ...
                    cmd.Parameters.Add(
                        new SqlParameter("@DESCRIPTION", (object)events.DESCRIPTION ?? DBNull.Value)
                        {
                            SqlDbType = System.Data.SqlDbType.NVarChar
                        }
                     );
                    cmd.Parameters.Add(
                        new SqlParameter("@MDESCRIPTION", (object)events.MDESCRIPTION ?? DBNull.Value)
                        {
                            SqlDbType = System.Data.SqlDbType.NVarChar
                        }
                     );
        ...
        public string InsertHashtagsM(HashtagsM hashtags)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(ConnectionString))
